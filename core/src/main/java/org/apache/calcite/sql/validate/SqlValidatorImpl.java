  ...
  protected @PolyNull SqlNode performUnconditionalRewrites(
    ...
    if (node instanceof SqlCall) {
      ...
      if (call.getOperator() instanceof SqlUnresolvedFunction) {
        ...
        final List<SqlOperator> overloads = SqlUtil.lookupOperatorsByParameterCount(opTab,
            function.getFunctionType(), SqlSyntax.FUNCTION, function.getNameAsId(),
            catalogReader.nameMatcher(), call.operandCount());
        if (overloads.size() == 1) {
