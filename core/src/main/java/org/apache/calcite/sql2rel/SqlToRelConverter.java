...
public class SqlToRelConverter {
  ...
  protected final void createAggImpl(
    ...
    try {
      ...
      // is allocated.
      findSubQueries(bb, selectList, RelOptUtil.Logic.TRUE_FALSE_UNKNOWN, false);
  ...
  protected class Blackboard implements SqlRexContext, SqlVisitor<RexNode>,
    ...
    @Override public RexNode convertExpression(SqlNode expr) {
      ...
      for (SubQuery subQuery : subQueryList) {
        if (subQuery.node == expr && subQuery.expr == null) {
          substituteSubQuery(this, subQuery);
          if (subQuery.expr != null) {
            return requireNonNull(subQuery.expr);
          }
        }
      }
